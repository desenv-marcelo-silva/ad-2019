...
app.get("/sorteio", async function(req, res) {
  ...
  for (var x = 0; x < tamanhoMaximo - 1; x++) {
    listaSorteada.push({
      id: listaAmigosSorteio[x].id,
      nome: listaAmigosSorteio[x].nome,
      amigo: listaAmigosSorteio[x + 1].nome,
      email: listaAmigosSorteio[x + 1].email
    });
  }
  listaSorteada.push({
    id: listaAmigosSorteio[x].id,
  ...
  listaSorteada.forEach(function(item) {
    ...
    const data = {
      from:
        "Mailgun Sandbox <postmaster@sandbox838c04dbb0a648af8f3fa09feed3c161.mailgun.org>",
      to: item.email,
...
app.delete("/amigo/:id", async (req, res) => {
  try {
    ...
    if (!amigo) res.status(404).send("Item não encontrado.");
    res.status(200).send();
  } catch (err) {
    res.status(500).send(err);
  }
